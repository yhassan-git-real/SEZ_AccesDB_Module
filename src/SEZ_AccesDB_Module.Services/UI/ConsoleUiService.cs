...
public static class ConsoleUiService
{
    ...
    // ─── Threshold Validation UI ──────────────────────────────────────────────

    /// <summary>
    /// Displays a structured threshold violation report and prompts the user
    /// to choose how to proceed. Only called when at least one split table
    /// exceeds the configured threshold.
    /// </summary>
    public static ThresholdAction PromptThresholdAction(ThresholdValidationResult validation)
    {
        AnsiConsole.WriteLine();
        AnsiConsole.Write(new Rule("[bold yellow]⚠  Threshold Violation Detected[/]").RuleStyle("yellow"));
        AnsiConsole.WriteLine();

        // ── Header panel ──────────────────────────────────────────────────────
        AnsiConsole.Write(
            new Panel(
                $"Split tables generated by SP [bold cyan]{Markup.Escape(validation.SpName)}[/] exceed " +
                $"the defined threshold [bold yellow]{validation.Threshold:N0}[/] rows.\n" +
                $"[grey]MS Access has a 2 GB file size limit — loading oversized tables may cause runtime failures.[/]")
            .Header("[bold yellow]  ⚠ Attention Required  [/]")
            .Border(BoxBorder.Rounded)
        ...
        var table = new Table()
            .Border(TableBorder.Rounded)
            .BorderColor(Color.Grey)
            .Title("[bold]Split Table Details[/]")
            .AddColumn(new TableColumn("[bold grey]#[/]").Centered().Width(4))
            .AddColumn(new TableColumn("[bold grey]Table Name[/]"))
            .AddColumn(new TableColumn("[bold grey]Row Count[/]").RightAligned())
            .AddColumn(new TableColumn("[bold grey]Status[/]").Centered());

        int idx = 0;
        foreach (var t in validation.Tables)
        {
            idx++;
            var status = t.ExceedsThreshold
                ? "[bold red]⚠ EXCEEDS[/]"
                : "[green]✓ OK[/]";
            var rowColor = t.ExceedsThreshold ? "red" : "green";

            table.AddRow(
                $"[grey]{idx}[/]",
                Markup.Escape(t.TableName),
                $"[{rowColor}]{t.RowCount:N0}[/]",
                status);
        }

        // Summary footer
        var totalRows = validation.Tables.Sum(t => t.RowCount);
        table.AddEmptyRow();
        table.AddRow(
            "",
            "[bold]Total[/]",
            $"[bold]{totalRows:N0}[/]",
            $"[bold]{validation.ViolationCount}[/] of [bold]{validation.Tables.Count}[/] exceed");
        ...
        var choice = AnsiConsole.Prompt(
            new SelectionPrompt<string>()
                .Title("[bold yellow]Select an action:[/]")
                .PageSize(5)
                .HighlightStyle(new Style(foreground: Color.Cyan1))
                .AddChoices(
                    "1. Process all tables (ignore threshold violations)",
                    "2. Process only tables within threshold (skip exceeding)",
                    "3. Cancel entire operation"));

        var action = choice[0] switch
        {
            '1' => ThresholdAction.ProcessAll,
            '2' => ThresholdAction.ProcessValidOnly,
            _   => ThresholdAction.CancelOperation
        };

        // Log the user's decision in console
        var actionLabel = action switch
        {
            ThresholdAction.ProcessAll      => "[yellow]Processing ALL tables (threshold overridden by user)[/]",
            ThresholdAction.ProcessValidOnly => "[cyan]Processing only tables within threshold[/]",
            ThresholdAction.CancelOperation  => "[red]Operation CANCELLED by user[/]",
            _ => ""
        };
